...
def ask_agree_participate_text(schedule):
    return f'''Участвуешь во встречах на следующей неделе? Бот будет подбирать пары в понедельник {day_month(schedule.next_week_monday())}.

/{PARTICIPATE_COMMAND} - участвую
/{PAUSE_WEEK_COMMAND} - пауза на неделю
/{PAUSE_MONTH_COMMAND} - пауза не месяц
